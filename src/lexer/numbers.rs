...
impl<'a> NumberScanner<'a> {
    ...
    pub fn scan_number(
        ...
        if first_digit == '0' && !self.is_at_end() {
            match self.peek() {
                ...
                '0'..='9' => {
                    self.error(
                        "Leading zeros not allowed in decimal literals".to_string(),
                        start_line,
                        start_column,
                        start_offset,
                    );
        ...
        if self.peek() == '.' {
                ...
                )
            {
                ...
                if self.peek() == 'e' || self.peek() == 'E' {
                    return self.scan_scientific_notation(
                        start_line,
                        start_column,
                        start_offset,
                        start_pos,
                ...
                return self.finalize_float_token(
                    start_line,
                    start_column,
                    start_offset,
                    start_pos,
    ...
    pub fn scan_float_starting_with_dot(
        &mut self,
        start_line: usize,
        start_column: usize,
        start_offset: usize,
    ...
    fn scan_hex_number(
        ...
        if let Ok(value) = i64::from_str_radix(hex_part, 16) {
            ...
            self.error(
                "Hexadecimal literal out of range".to_string(),
                start_line,
                start_column,
                start_offset,
            );
    ...
    fn scan_octal_number(
        ...
        if let Ok(value) = i64::from_str_radix(octal_part, 8) {
            ...
            self.error(
                "Octal literal out of range".to_string(),
                start_line,
                start_column,
                start_offset,
            );
    ...
    fn scan_binary_number(
        ...
        if let Ok(value) = i64::from_str_radix(binary_part, 2) {
            ...
            self.error(
                "Binary literal out of range".to_string(),
                start_line,
                start_column,
                start_offset,
            );
