        ...
        match c {
            ...
            '/' => {
                if self.match_char('/') {
                    let mut comment_scanner = CommentScanner::new(
                        self.input, self.position, self.line, self.column, self.byte_offset, self.errors
                    );
            ...
            c if c.is_ascii_alphabetic() || c == '_' => {
                let mut identifier_scanner = IdentifierScanner::new(
                    self.input, self.position, self.line, self.column, self.byte_offset, self.keywords
                );
