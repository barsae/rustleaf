...
impl Evaluator {
    ...
    fn get_attribute(&mut self, obj: Value, attr_name: &str) -> Result<Value> {
        ...
        match &obj {
            Value::Int(n) => n.get_attr(attr_name).ok_or_else(|| {
                anyhow::anyhow!(format!("'int' object has no attribute '{}'", attr_name))
            }),

            Value::Float(f) => f.get_attr(attr_name).ok_or_else(|| {
                anyhow::anyhow!(format!("'float' object has no attribute '{}'", attr_name))
            }),
            ...
            Value::String(s) => {
                s.get_attr(attr_name)
                    ...
                    .ok_or_else(|| {
                        anyhow::anyhow!(format!("'string' object has no attribute '{}'", attr_name))
                    })
            }

            _ => Err(anyhow::anyhow!(format!(
                "Attribute access not yet implemented for {:?}",
                obj
            ))),
        }
    }
    ...
    fn set_attribute(&mut self, _obj: Value, _attr_name: &str, _value: Value) -> Result<Value> {
        ...
        Err(anyhow::anyhow!("Attribute setting not yet implemented"))
    }

    fn call_method(&mut self, method: Value, args: Vec<Value>) -> Result<Value> {
        match method {
            Value::BuiltinMethod(builtin) => self.call_builtin_method(builtin, None, args),
    ...
    fn call_builtin_method(
        ...
        match method {
            BuiltinMethod::IntAdd => {
                if args.len() != 1 {
                    return Err(anyhow::anyhow!(format!(
                        "op_add() takes exactly 1 argument ({} given)",
                        args.len()
                    )));
                }
            ...
            _ => Err(anyhow::anyhow!(format!(
                "Builtin method {:?} not yet implemented",
                method
            ))),
        }
    }
