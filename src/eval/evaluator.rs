...
impl Evaluator {
    ...
    /// Get a variable from the global scope with Result-based error handling
    ...
    pub fn call(&mut self, function: Value, args: &[Value]) -> anyhow::Result<Value> {
        // Use the Args system to call the function directly
        use crate::core::Args;

        let args_obj = Args::positional(args.to_vec());
        function
            .call(args_obj)
