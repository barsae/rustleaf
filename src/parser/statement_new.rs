...
pub fn parse_statement(s: &mut TokenStream) -> Result<Statement> {
    ...
    if let Some(stmt) = s.try_parse(parse_macro)? {
        return trace_exit!("parse_statement", Ok(stmt));
    }
...
fn parse_macro(s: &mut TokenStream) -> Result<Statement> {
    ...
    trace_exit!("parse_macro", Ok(result))
}
...
fn parse_var_declaration(s: &mut TokenStream) -> Result<Statement> {
    trace_enter!("parse_var_declaration");
    ...
    trace_exit!("parse_var_declaration", Ok(Statement::VarDecl { pattern, value }))
}

fn parse_function_declaration(s: &mut TokenStream) -> Result<Statement> {
    trace_enter!("parse_function_declaration");
    ...
    let body_expr = super::expression_new::parse_block_expression(s)?;
    let body = match body_expr {
        Expression::Block(block) => block,
        _ => unreachable!("parse_block_expression should return a Block"),
    };
    
    trace_exit!("parse_function_declaration", Ok(Statement::FnDecl {
        name,
        params,
        body,
        is_pub,
...
fn parse_class_declaration(s: &mut TokenStream) -> Result<Statement> {
    trace_enter!("parse_class_declaration");
    let is_pub = s.accept_type(TokenType::Pub)?.is_some();
    ...
    trace_exit!("parse_class_declaration", Ok(Statement::ClassDecl {
        name,
        members,
        is_pub,
    }))
...
fn parse_import_statement(s: &mut TokenStream) -> Result<Statement> {
    trace_enter!("parse_import_statement");
    ...
    trace_exit!("parse_import_statement", Ok(Statement::Import(ImportSpec { module, items })))
}

fn parse_return_statement(s: &mut TokenStream) -> Result<Statement> {
    trace_enter!("parse_return_statement");
    ...
    s.expect_type(TokenType::Semicolon)?;
    trace_exit!("parse_return_statement", Ok(Statement::Return(expr)))
}

fn parse_break_statement(s: &mut TokenStream) -> Result<Statement> {
    trace_enter!("parse_break_statement");
    ...
    trace_exit!("parse_break_statement", Ok(Statement::Break(expr)))
}

fn parse_continue_statement(s: &mut TokenStream) -> Result<Statement> {
    trace_enter!("parse_continue_statement");
    s.expect_type(TokenType::Continue)?;
    s.expect_type(TokenType::Semicolon)?;
    trace_exit!("parse_continue_statement", Ok(Statement::Continue))
}

fn parse_assignment(s: &mut TokenStream) -> Result<Statement> {
    trace_enter!("parse_assignment");
    ...
    trace_exit!("parse_assignment", Ok(Statement::Assignment { target, op, value }))
}
...
fn parse_expression(s: &mut TokenStream) -> Result<Expression> {
    super::expression_new::parse_expression(s)
}
