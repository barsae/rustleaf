...
pub fn print(args: Vec<Value>) -> Result<Value> {
    let strings: Vec<String> = args.iter().map(super::to_string).collect();
...
pub fn int(args: Vec<Value>) -> Result<Value> {
    ...
    match &args[0] {
        ...
        Value::String(s) => s
            .parse::<i64>()
            .map(Value::Int)
            .map_err(|_| anyhow::anyhow!(format!("invalid literal for int(): '{}'", s))),
        _ => Err(anyhow::anyhow!(
...
pub fn float(args: Vec<Value>) -> Result<Value> {
    ...
    match &args[0] {
        ...
        Value::String(s) => s
            .parse::<f64>()
            .map(Value::Float)
            .map_err(|_| anyhow::anyhow!(format!("invalid literal for float(): '{}'", s))),
        _ => Err(anyhow::anyhow!(
...
pub fn is_unit(args: Vec<Value>) -> Result<Value> {
    if args.len() != 1 {
        return Err(anyhow::anyhow!(format!(
            "is_unit() takes exactly 1 argument ({} given)",
            args.len()
        )));
    }
...
pub fn range(args: Vec<Value>) -> Result<Value> {
    ...
    Err(anyhow::anyhow!("range() not yet implemented".to_string()))
}
...
pub fn assert(args: Vec<Value>) -> Result<Value> {
    ...
    let is_true = match condition {
        Value::Bool(b) => *b,
        Value::Null => false,
        _ => return Err(anyhow::anyhow!("Only null and bool have truthiness")),
    };
...
pub fn raise(args: Vec<Value>) -> Result<Value> {
    if args.len() != 1 {
        return Err(anyhow::anyhow!(format!(
            "raise() takes exactly 1 argument ({} given)",
            args.len()
        )));
    }
