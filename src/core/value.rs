...
impl Value {
    ...
    pub fn as_mut_ref_vec_value(&self) -> Result<std::cell::RefMut<Vec<Value>>> {
        match self {
            ...
            _ => Err(anyhow!(
                "Cannot mutably borrow {} as &mut Vec<Value>",
                self.type_name()
            )),
        }
    }
    ...
    pub fn as_mut_ref_indexmap_string_value(
        ...
        match self {
            ...
            _ => Err(anyhow!(
                "Cannot mutably borrow {} as &mut IndexMap<String, Value>",
                self.type_name()
            )),
        }
    }
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Value::ClassInstance(instance) => {
                ...
                write!(f, "ClassInstance({})", inst.class_name)
            }
            Value::Range(range) => write!(f, "Range({range:?})"),
            // Avoid infinite recursion by not debug printing the internal RustValue
            Value::RustValue(rust_val) => match rust_val.type_name() {
                Some(name) => write!(f, "RustValue(<{name}>)"),
                None => write!(f, "RustValue(<unknown>)"),
            },
            Value::Raised(value) => write!(f, "Raised({value:?})"),
        }
    }
}
